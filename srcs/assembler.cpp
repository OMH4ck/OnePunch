...
namespace onepunch {
  ...
  std::vector<SegmentPtr> Assembler::GetCallSegment(std::vector<InstrPtr>& insts) {
    int start = 0;
    ...
    for (int idx = 0; idx < size; idx++) {
      ...
      if (inst->opcode_ == OPCODE::OP_CALL || inst->opcode_ == OPCODE::OP_JMP) {
        if (!inst->op_dst_.has_value() || IsInteresting(inst->op_dst_.value()) == false
        ...
        while (start < idx + 1) {
          auto tmp_inst = insts[start];
          if (IsUnwantedInstructions(tmp_inst->opcode_) || (tmp_inst->operation_length_ != kQWORD)
              || (tmp_inst->op_dst_

...
void Minimizer::MinimizeSegmentNb(
    int idx, std::vector<std::pair<SegmentPtr, unsigned>>& run_code,
...
bool Minimizer::RunSegmentList(
  ...
  for (auto& segment_info : run_code) {
    ...
    if (executor.ExecuteInstructions(segment, registers, false) == false)
      return false;
  }
